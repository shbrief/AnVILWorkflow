...
dim(f_all) 
length(unique(f_ws$workspace_key)) # only 2 results
length(unique(f_dat$workspace_key)) # 37 results
allkeys <- unique(c(f_ws$workspace_key,
                    f_wf$workspace_key,
                    f_dat$workspace_key))

sum(!f_all$workspace_key %in% allkeys)
sum(!allkeys %in% f_all$workspace_key)
```
